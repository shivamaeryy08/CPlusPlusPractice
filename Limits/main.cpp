...
LIMITS
Floating pt:
    min -> smallest possible pos number of that type
    max -> biggest number of that type
    lowest -> lowest negative number of htat typt
Signed integer:
    lowest -> n/a
    min -> smallest negative number only negative if signed, for unsigned min is 0
    max -> biggest pos number
*/